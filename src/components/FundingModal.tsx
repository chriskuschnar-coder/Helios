...
export function FundingModal({ isOpen, onClose, prefilledAmount, onProceedToPayment }: FundingModalProps) {
  ...
  if (!selectedMethod) {
    ...
    // Show empty portfolio state if no amount selected and balance is 0
    if (!numericAmount && (!account || account.balance === 0)) {
      return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <EmptyPortfolioState onFundAccount={() => setAmount('1000')} />
          </div>
        </div>
      )
    }
