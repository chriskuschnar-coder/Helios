...
export function PortfolioValueCard({ onFundPortfolio, onWithdraw, kycStatus = 'unverified' }: PortfolioValueCardProps) {
  ...
  // Withdraw should only be disabled if KYC is not verified
  const shouldDisableWithdraw = !isKYCVerified
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg border border-gray-100 p-3 sm:p-4 md:p-8 mb-3 sm:mb-4 md:mb-6 mobile-card">
      ...
      <div className="flex flex-col xs:flex-row gap-2 sm:gap-3 mobile-space-y-1">
        {/* Fund Portfolio - Always enabled to allow onboarding flow */}
        <button
        ...
        {/* Withdraw - Disabled until KYC verified */}
        <button
          onClick={shouldDisableWithdraw ? undefined : onWithdraw}
          ...
          title={shouldDisableWithdraw ? 'Complete identity verification to enable withdrawals' : ''}
        >
